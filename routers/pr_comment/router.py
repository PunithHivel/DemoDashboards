...
from repositories.pr_comment_repository import PR_COMMENT_COLUMNS, PrCommentRepository
from schemas.pr_comment import (
    BOOL_FIELDS,
    DATETIME_FIELDS,
    DEFAULT_PR_COMMENT_VALUES,
    INT_FIELDS,
    REQUIRED_FIELDS,
    PrCommentImportResponse,
)
...
router = APIRouter(prefix="/pr-comment", tags=["pr-comment"])
_repository = PrCommentRepository()
...
def _prepare_rows(df: pd.DataFrame) -> List[Dict]:
    ...
    for idx, record in enumerate(records, start=1):
        try:
            row: Dict[str, Any] = {}
            for column in PR_COMMENT_COLUMNS:
                ...
                raw_value = sanitize_value(
                    record.get(column, DEFAULT_PR_COMMENT_VALUES.get(column))
                )
...
@router.post(
    ...
    summary="Import PR comments from a CSV file",
    response_model=PrCommentImportResponse,
)
async def import_pr_comments_from_csv(
    ...
    if not rows:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No valid PR comment rows found in CSV.",
        )
