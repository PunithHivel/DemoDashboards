from __future__ import annotations

from datetime import datetime
from typing import Set

from fastapi import APIRouter, Depends, File, HTTPException, UploadFile, status
from sqlalchemy.orm import Session

from db.session import get_db_session
from repositories.cursor_spending_repository import (
    CURSOR_SPENDING_COLUMNS,
    CursorSpendingRepository,
)
from routers._csv_import_helpers import prepare_rows
from schemas.ai_import import GenericImportResponse
from utils.csv_loader import read_uploaded_csv

router = APIRouter(prefix="/cursor-spending", tags=["cursor-spending"])
_repository = CursorSpendingRepository()

REQUIRED_FIELDS: Set[str] = {"organization_id", "email"}
INT_FIELDS: Set[str] = {
    "organization_id",
    "user_integration_id",
    "author_id",
    "spend_cents",
    "fast_premium_requests",
    "hard_limit_override_dollars",
    "original_author_id",
}
DATETIME_FIELDS: Set[str] = {
    "subscription_cycle_start",
    "created_at",
    "updated_at",
    "first_seen_in_billing_cycle",
    "last_seen_in_billing_cycle",
}


@router.post(
    "/csv",
    status_code=status.HTTP_201_CREATED,
    summary="Import cursor_spending from CSV",
    response_model=GenericImportResponse,
)
async def import_csv(
    file: UploadFile = File(...),
    session: Session = Depends(get_db_session),
) -> GenericImportResponse:
    df = await read_uploaded_csv(file)

    rows = prepare_rows(
        df,
        columns=CURSOR_SPENDING_COLUMNS,
        required_fields=REQUIRED_FIELDS,
        int_fields=INT_FIELDS,
        datetime_fields=DATETIME_FIELDS,
    )
    if not rows:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No valid rows found in CSV.")

    now = datetime.utcnow()
    for row in rows:
        row["created_at"] = row.get("created_at") or now
        row["updated_at"] = row.get("updated_at") or now

    inserted = _repository.bulk_insert(session, rows)
    inserted_count = inserted[0].get("inserted_count", 0) if inserted else 0
    return GenericImportResponse(
        rows_received=len(df),
        rows_inserted=inserted_count,
        rows_skipped=len(df) - len(rows),
    )
